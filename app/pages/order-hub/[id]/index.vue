...
onMounted(() => {
  if (!loading.value && user && bookingCode)
...
=======
      <BasePill bg="bg-[#F6F6FA]">
        ...
        <span class="font-bold text-[#1E1E1E]">{{ bookingCode }}</span>
      </BasePill>
    ...
    <template v-if="loading && bookingProgressLoading">
      ....
    </template>
>>>>>>> Refactor page structure; Page customer infor and item & packages
...
=======
          <UICardInformation :status="(bookingData as any)?.customer_information?.status as any"
            :totalProgress="Number((bookingData as any)?.customer_information?.progress_count?.total || 0)"
            :currentProgress="Number((bookingData as any)?.customer_information?.progress_count?.completed || 0)"
            :title="['CUSTOMER', 'INFORMATION']" :icon="IconOrderHubCustomer"
            description="Personal details, contact info, and addresses." @action="navigateTo('customer-information')" />

          <!-- ITEM & PACKAGES -->
          <UICardInformation :status="(bookingData as any)?.item_and_package?.status as any"
            :totalProgress="Number((bookingData as any)?.item_and_package?.progress_count?.total || 0)"
            :currentProgress="Number((bookingData as any)?.item_and_package?.progress_count?.completed || 0)"
            :title="['ITEM &', 'PACKAGES']" :icon="IconOrderHubItemAndPackages"
            description="Details of items, weights and packaging types." @action="navigateTo('item-and-packages')" />


          <!-- COMPLIANCE DOCUMENT -->
          <UICardInformation :status="(bookingData as any)?.compliance_document?.status as any"
            :totalProgress="Number((bookingData as any)?.compliance_document?.progress_count?.total || 0)"
            :currentProgress="Number((bookingData as any)?.compliance_document?.progress_count?.completed || 0)"
            :title="['COMPLIANCE', 'DOCUMENT']" :icon="IconOrderHubCustomer"
            description="Submit document needed for your international move."
            @action="navigateTo('compliance-document')" />


          <!-- PICKUP DETAIL & SCHEDULE: LOCKED -->
          <UICardInformation :status="(bookingData as any)?.pickup_detail_schedule?.status as any"
            :totalProgress="Number((bookingData as any)?.pickup_detail_schedule?.progress_count?.total || 0)"
            :currentProgress="Number((bookingData as any)?.pickup_detail_schedule?.progress_count?.completed || 0)"
            :title="['PICKUP DETAIL &', 'SCHEDULE']" :icon="IconOrderHubPickup"
            description="Select your preferred date & time for pickup."
            @action="navigateTo('pickup-detail-schedule')" />
        </div>

        <div>
          <p class="text-[14px] leading-[22px] text-[#404040] font-[400]">
            Packing List Code: <span class="font-bold">{{ packingListCode || "-" }}</span>
>>>>>>> Refactor page structure; Page customer infor and item & packages
          </p>
        </div>
      </div>
