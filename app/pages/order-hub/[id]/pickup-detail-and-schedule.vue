<script setup lang="ts">
  ...
  const { bookingCode, navigateToOrderHub } = useOrderHub();
  ...
  const buildingTypeOptions = [
    ...
    { name: "Apartement / Flat", code: "apartement_of_flat" },
  ];
      ...
      .superRefine((data, ctx) => {
        if (data.buildingType?.code === "apartement_of_flat") {
          if (!data.hasFreightElevator) {
            ctx.addIssue({ path: ["hasFreightElevator"], message: "Please select elevator availability" });
          }
          if (!data.hasReceptionist) {
            ctx.addIssue({ path: ["hasReceptionist"], message: "Please select receptionist availability" });
          }
        }
      }),
  );
  ...
  /**
   * Submit pickup detail & schedule.
   * Fixes:
   *  1. Endpoint was wrong (/order-hub/item-and-package → /api/order-hub/pickup-detail-and-schedule)
   *  2. buildingType was sent as {code, name} object — BE expects string
   *  3. success state was set before API call — moved to after success
   */
  const handleSubmit = async ({ values, valid }: { values: Record<string, unknown>; valid: boolean }) => {
    ...
    // Normalize buildingType: FE stores {code, name} object, BE expects string code
    const buildingTypeRaw = values.buildingType as { code?: string } | string | null;
    const buildingType =
      typeof buildingTypeRaw === "object" && buildingTypeRaw !== null
        ? buildingTypeRaw.code
        : buildingTypeRaw;
    ...
    try {
      const res = await $fetch("/api/order-hub/pickup-detail-and-schedule", {
        method: "PUT",
        body: payload,
        credentials: "include",
      });
      ...
      errorSubmit.value = e?.data?.message || "Error submitting pickup detail and schedule";
    } finally {
  ...
  <section class="flex flex-col relative gap-6 max-w-7xl mx-auto px-6 pt-28 pb-24">
    <template v-if="viewMode === 'form'">
      ...
      <section class="w-[1024px] mx-auto flex flex-col gap-6 mt-4">
        ...
        <!-- Error Banner -->
        <div v-if="errorSubmit" class="px-4 py-3 bg-red-50 border border-red-200 rounded-lg text-red-600 text-sm">
          {{ errorSubmit }}
        </div>
              ...
              <div class="flex flex-col gap-6">
                ...
                <div class="relative flex flex-col gap-[6px]">
                  ...
                  <Message v-if="$form.shipperFullName?.invalid" severity="error" size="small" variant="simple">
                    {{ $form.shipperFullName.error?.message }}
                  </Message>
                </div>
                ...
                <div class="relative flex flex-col gap-[6px]">
                  ...
                  <Message v-if="$form.buildingType?.invalid" severity="error" size="small" variant="simple">
                    {{ $form.buildingType.error?.message }}
                  </Message>
                  ...
                  <div v-if="isApartment" class="flex flex-col gap-6 p-4 bg-neutral-20">
                    ...
                    <div class="flex flex-col gap-2">
                      <div class="flex flex-col gap-1">
                        <label class="font-medium text-neutral-90">
                          Is there a freight elevator access? <span class="text-red-500">*</span>
                        </label>
                        <p class="text-[12px] text-neutral-60">
                          Please inform us about the availability of a freight elevator for pickup at the apartment / flat.
                        </p>
                      </div>
                      <div class="flex gap-6 mt-2">
                        <div class="flex items-center gap-2 cursor-pointer">
                          <RadioButton name="hasFreightElevator" input-id="freight_yes" value="yes" />
                      ...
                      <Message v-if="$form.hasFreightElevator?.invalid" severity="error" size="small" variant="simple">
                    ...
                    <div class="flex flex-col gap-2">
                      <div class="flex flex-col gap-1">
                        <label class="font-medium text-neutral-90">
                          Is there a receptionist? <span class="text-red-500">*</span>
                        </label>
                        <p class="text-[12px] text-neutral-60">
                          If the pickup address is an apartment / flat, we strongly recommend that the package be left with the receptionist.
                        </p>
                      </div>
                      <div class="flex gap-6 mt-2">
                        <div class="flex items-center gap-2 cursor-pointer">
                          <RadioButton name="hasReceptionist" input-id="receptionist_yes" value="yes" />
                          <label for="receptionist_yes">Yes</label>
                        </div>
                        <div class="flex items-center gap-2 cursor-pointer">
                          <RadioButton name="hasReceptionist" input-id="receptionist_no" value="no" />
                          <label for="receptionist_no">No</label>
                        </div>
                      </div>
                      <Message v-if="$form.hasReceptionist?.invalid" severity="error" size="small" variant="simple">
                ...
                <div class="relative flex flex-col gap-[6px]">
                  ...
                  <Message v-if="$form.streetAddress?.invalid" severity="error" size="small" variant="simple">
                ...
                <!-- PIC for Pickup Phone Number -->
                <div class="relative flex flex-col gap-[6px]">
                  <label class="font-medium text-neutral-90">PIC for Pickup Phone Number [Origin]</label>
                  <div class="relative">
                    <InputIcon
                  ...
                  <Message v-if="$form.shipperOriginPhoneNumber?.invalid" severity="error" size="small" variant="simple">
            ...
            <div class="px-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div class="flex flex-col gap-3">
                ...
                <DatePicker
                  ...
                  :min-date="new Date()"
                  class="w-full"
                  :class="$form.pickupDate?.invalid ? 'border border-red-500 rounded-[8px]' : ''"
              ...
              <div class="flex flex-col gap-3">
                ...
                >
                  <template #option="slotProps">
                    ...
                    >
                      <IconSun />
                      <span class="text-body-lg text-neutral-90">{{ slotProps.option.label }}</span>
                    </div>
                  </template>
                </SelectButton>
