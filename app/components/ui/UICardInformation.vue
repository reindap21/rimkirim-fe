...
<template>
  <div class="rounded-[16px] px-6 py-4 border border-[#EDEDED] flex flex-col justify-between gap-8"
    ...
    <div class="h-full flex flex-col gap-4">
      ...
      <div class="flex items-center justify-end h-[38px] mt-auto">
        <button class="text-[14px] leading-[22px] font-medium text-neutral-100 flex items-center gap-1 hover:opacity-80"
          v-if="status !== 'locked'" @click="emit('action')">
          {{ status === "awaiting_input" ? "Start" : "Edit" }} →
        </button>
        <button class="text-[14px] leading-[22px] font-medium text-neutral-70 flex items-center gap-1 hover:opacity-80"
          v-else>
